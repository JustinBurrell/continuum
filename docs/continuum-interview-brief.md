...
| Email | Resend — transactional (`noreply@usecontinuum.dev`) + digest (`hello@usecontinuum.dev`); centralised in `email.service.js` |
...
- **Email notifications** — two-tier system via Resend. Tier 1 transactional: welcome on register, new device login alert (fires only when no active session exists for the deviceId before revocation), Google Drive connected/disconnected (modular config in `integrations.js` — future integrations add a key, no controller changes), account deletion with one-click SHA-256-hashed restore token, account restored confirmation. Tier 2 digest: smart daily (`node-cron`, 8AM UTC, queries Notification collection for undigested friend requests / shares / assignments) and weekly summary (6PM UTC Sunday, covers comments, likes, friend activity, study streak, stale applications). In-process cron scheduling — chosen over a Redis-backed job queue (BullMQ) because constant queue polling exhausted Upstash's free-tier request quota for what are just three fixed-time daily/weekly jobs. One-click unsubscribe via signed JWT (`EMAIL_UNSUBSCRIBE_SECRET`, separate from `JWT_SECRET`). Digest suppression query excludes `isDemo`, `isSeedUser`, `pendingDeletion`, and users inactive 30+ days — no seed/demo accounts ever receive digest emails.
