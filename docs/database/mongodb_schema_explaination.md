...
**Key behaviors:**
- `unique: true` on `email` with MongoDB handling duplicates at the DB layer (E11000)
- Controller catches duplicates and returns `200` (idempotent — avoids leaking "this email is registered")
- No `userId` reference — intentionally anonymous pre-registration signups
- No TTL — entries are permanent until the mobile app launches and the list is exported

**Query surface:** Admin reads only (no user-facing reads). Exports via MongoDB Atlas or a future admin endpoint.

---

### 9. **Multi-Device Auth via RefreshToken Collection**
...
- `deviceId` — human-readable label parsed from User-Agent (e.g. `"Chrome 120 on macOS 10.15.7"`, `"Safari 17 on iPhone (iOS 17)"`, `"Pixel 8 on Android 15"`). The Android app sends a descriptive User-Agent header including the device model and OS version via the `Build` class, matching the same labeling pattern used by web browsers.
...
- `lastUsedAt` — timestamp updated each time the token is used to issue a new JWT via `POST /auth/refresh` or `POST /auth/mobile/refresh`

The JWT payload embeds `sessionId` (the `RefreshToken._id`). Auth middleware checks a Redis blocklist key `revoked_session:{sessionId}` to immediately reject active JWTs for revoked sessions — no need to wait for the 1-day access token expiry. This path is fail-open: if Redis is unavailable the check is skipped and the existing `tokenVersion` + `revokedAt` guards still apply. `logout-all` uses `tokenVersion` increment for immediate cross-device invalidation.

**Refresh token rotation:** On every `POST /auth/refresh` (web) or `POST /auth/mobile/refresh` (Android) call, the old `RefreshToken` is soft-revoked, its `sessionId` is written to the Redis blocklist (so any still-valid JWT bearing it is immediately rejected), and a new `RefreshToken` document is created inheriting `deviceId`, `ipAddress`, and `ipLocation`. This means a stolen refresh token can only be used once — rotation invalidates it immediately on first use. Session continuity in the Security tab UI is preserved because the new document carries forward the same device and location metadata.

**Mobile logout:** `POST /auth/mobile/logout` accepts the refresh token in the request body (since Android stores tokens in EncryptedSharedPreferences, not cookies). The controller hashes the token with SHA-256, finds the matching `RefreshToken` document, and hard-deletes it. The Android client then clears local storage and navigates to the login screen. If another device revokes the session remotely, the Android app's `TokenAuthenticator` detects the 401 during the next API call or token refresh, broadcasts a `LogoutReason.REMOTE_INVALIDATION` event via SharedFlow, and `AppNavHost` navigates to login with an explanatory message.
